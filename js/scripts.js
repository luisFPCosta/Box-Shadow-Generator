class BoxShadowGenerator {
	...
	applyRule() {
		const rgbColor = this.hexToRgb(this.colorRef.value);
		...
		}px rgba(${rgbColor}, ${this.opacityRef.value})`;
...
horizontalRef.addEventListener("keyup", (e) => {
	let value = validateNumbers(horizontalRef.value);
...
verticalRef.addEventListener("keyup", (e) => {
	let value = validateNumbers(verticalRef.value);
	if (e.key === "Enter" || e.keyCode === 13) {
		if (value > 100) {
			value = 100;
		} else if (value < -100) {
...
blurRef.addEventListener("keyup", (e) => {
	let value = validateNumbers(blurRef.value);
	if (e.key === "Enter" || e.keyCode === 13) {
		if (value > 100) {
			value = 100;
		} else if (value < 0) {
...
spreadRef.addEventListener("keyup", (e) => {
	let value = validateNumbers(spreadRef.value);
	if (e.key === "Enter" || e.keyCode === 13) {
		if (value > 100) {
			value = 100;
		} else if (value < -100) {
...
opacityRef.addEventListener("keyup", (e) => {
	let value = validateNumbers(opacityRef.value);
	if (e.key === "Enter" || e.keyCode === 13) {
		if (value > 1) {
			value = 1;
		} else if (value < 0) {
...
const configureLanguage = () => {
	if (
		...
		!document.URL.includes("index_pt-br.html")
	) {
